...
const AttendenceHistroy = ({ isOpen, closeModal, isHistroyId ,datafor}) => {
  const apiUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
  const [actionDetails, setDetails] = useState([]);
  
  const getAttendanceDetails = async (idH) => {
    try {
      
      const response = await axiosJWT.get(`${apiUrl}/permission/viewHistoryMenu`, { params: { id: idH , isFor:datafor} });
