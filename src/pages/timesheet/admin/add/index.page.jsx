...
import GanttChart from './Gannt-chart.jsx';
import { assignDatesToTasks, createRowFromFields, reactSelectStyles } from "./dateUtils";
...
export default function TableWithField({ userFormdata }) {  // Default to empty array if not provided
  ...
  const handleDataChange = async (rowIndex, field, value, meta = {}) => {
    ...
    if (field === "startDate" || field === "endDate") {
      ...
      // 🔹 sync subtasks when parent date changes
      if (Array.isArray(updatedData[rowIndex].subTasks)) {
        updatedData[rowIndex].subTasks =
          updatedData[rowIndex].subTasks.map(sub => {
            let updatedSub = { ...sub };

            // parent start shortened
            if (
              field === "startDate" &&
              sub.startDate &&
              new Date(sub.startDate) < new Date(newDate)
            ) {
              updatedSub.startDate = newDate;
            }

            // parent end shortened
            if (
              field === "endDate" &&
              sub.endDate &&
              new Date(sub.endDate) > new Date(newDate)
            ) {
              updatedSub.endDate = newDate;
            }
    ...
    const calculateTotals = async () => {
      ...
      const updatedData = await Promise.all(
        data.map(async (row) => {
          ...
          // 🔹 parent percentage
          percentageTotal += Number(row.taskPercentage) || 0;

          // 🔹 parent assigned users
          if (Array.isArray(row.assignedTo)) {
            row.assignedTo.forEach(emp => {
              if (emp?.idEmployee) employeeSet.add(emp.idEmployee);
            });
          }

          // 🔹 parent days
          if (row.startDate && row.endDate) {
            const parentDays = await countWorkingDays(
              row.startDate,
              row.endDate
            );

            daysTotal += parentDays;

            if (row.noOfDays !== parentDays) {
              updatedRow.noOfDays = parentDays;
              hasChange = true;
            }
          }
          ...
          if (Array.isArray(row.subTasks)) {
            updatedRow.subTasks = await Promise.all(
              row.subTasks.map(async (sub) => {
                let updatedSub = { ...sub };

                if (sub.startDate && sub.endDate) {
                  const subDays = await countWorkingDays(
                    sub.startDate,
                    sub.endDate
                  );
                  ...
                  if (sub.noOfDays !== subDays) {
                    updatedSub.noOfDays = subDays;
                    hasChange = true;
                  }
                }
  ...
  const validateFields = (fields) => {
    ...
    data.forEach((row, rowIndex) => {
      ...
      // 🔹 Validate subtasks
      if (Array.isArray(row.subTasks)) {
        row.subTasks.forEach((subTask, subIndex) => {
          fields.forEach(field => {
            field.validations?.forEach(validation => {
              if (
                validation.type === "required" &&
                (!subTask[field.name] || subTask[field.name].length === 0)
              ) {
                if (!newErrors[rowIndex]) newErrors[rowIndex] = {};
                if (!newErrors[rowIndex].subTasks) {
                  newErrors[rowIndex].subTasks = {};
                }
                if (!newErrors[rowIndex].subTasks[subIndex]) {
                  newErrors[rowIndex].subTasks[subIndex] = {};
                }

                newErrors[rowIndex].subTasks[subIndex][field.name] =
                  validation.message;
              }
            });
          });
                                          ...
                                          <>
                                            {tasks.length === 0 ? (
                                              ...
                                              <div className="timesheet-gantt">
                                                ...
                                                <div className="col-12 display-top-time">
                                                  <div className='total-time-summary-all gantt-chart-top-box'>
                                                    ...
                                                    <div className='total-time-summary-days'>
                                                      ...
                                                      <div className='sub-summ-d-main'>
                                                        <p><b>Duration</b></p>
                                                        <p className='sub-summ-d'> {formatDate(previousformvalue.startDate)} - {formatDate(previousformvalue.endDate)}</p>
                                                      </div>
                                                    </div>
                                                    <div className='total-time-summary-days'>
                                                      <div className='sub-summ-d-icon'>
                                                ...
                                                <GanttChart dataEntry={data} projectid={previousformvalue.idProject || ""} />
                                              </div>
                                            )}
                                            <div className="justify-content-end d-flex w-100 mt-4">
