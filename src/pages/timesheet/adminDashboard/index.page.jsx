...
export default function adminDashboard({ }) {
    ...
    const fetchData = async () => {
        try {
            const response = await axiosJWT.get(`${apiUrl}/timesheet/list`);
            if (response) {
                const timesheetColoums = response.data.data.formcolumns;
                const timesheetData = response.data.data.timesheetData;
                setDatacoloum(timesheetColoums);
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        setProjectOptions(optionsData);
        setProjectvalue(optionsData.length > 0 ? optionsData[0].value : "")
      } catch (error) {
    ...
    const onChangePrroject = (value) => {
        if (value !== null) {
            setProjectvalue(value.value); // Update active tab index when a tab is clicked
        } else {
            setProjectvalue();
        }
    };
    ...
    useEffect(() => {
        if (setMouth && setYear) {
            const getgraphData = async () => {
                try {
                    const response = await axiosJWT.get(`${apiUrl}/timesheet/getAdminChart`,
                        {
                            params: { "idProject":projectvalue, "month": setMouth, "year": setYear, "isFor": "chats" }
                        });
                    // Handle the response if needed
                    if (response) {
            ...
            const getStatsData = async () => {
                try {
                    const response = await axiosJWT.get(`${apiUrl}/timesheet/getAdminChart`,
                        {
                            params: { "idProject":projectvalue, "month": setMouth, "year": setYear, "isFor": "stats" }
                        });
                    // Handle the response if needed
                    if (response) {
    ...
    }, [setMouth, setYear, projectvalue]);
    return (
        <>
            <Head>
        <title>{pageTitles.TimesheetTaskDashboard}</title>
        <meta name="description" content={pageTitles.TimesheetTaskDashboard} />
    </Head>
            ...
            <div className="main-wrapper">
                <div className="page-wrapper">
                    <div className="content container-fluid">
                        ...
                        <Breadcrumbs maintext={"Task Dashboard (Admin)"} addlink={"/TimeManagement"}/>
                                                        ...
                                                        <div className="row">
                                                            <div className="col-md-4">
                                                                <div className="form-group">
                                                                    <SelectComponent label={"Filter Data by Project"} placeholder={"Select Year..."} options={projectoptions} onChange={onChangePrroject} value={projectvalue} />
                                                                </div>
                                                            </div>
                                                            <div className="col-md-4">
                                                                <div className="form-group">
