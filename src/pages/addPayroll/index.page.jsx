...
export default function addPayroll({ payrollForm }) {
    ...
    const getsubmitformdata = async (newArray, value) => {
        const updatedArray = {
            ...newArray,
            section: newArray.section.map((section) => {
        ...
        const Payload = {
            ...updatedArray,
            section: updatedArray.section.map((section) => {
...
export async function getServerSideProps(context) {
    ...
    const apiUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
    const payrollForm = await fetchWithToken(`${apiUrl}/getDynamicForm`, { formType: 'add_payroll' }, context);
    return {
        props: { payrollForm },
    }
}
