...
export default function createClientNew({ userFormdata }) {  // Default to empty array if not provided
	...
	const [existClienterror, setExistClientError] = useState("");
       ...
       const submitformdata = async (value) => {
        ...
        if (response) {
            if (response.data.status === 400) {
                setExistClientError(response.data.data)
            } else {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response) {
                if (activeTab === "Address Information") {
                    const message = 'You have successfully Added New <strong>client</strong>!';
                    toast.success(({ id }) => (
