  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      try {
        ...
        const roleInfoSubsection = data.Subsection.find(subsection => subsection.SubsectionName === 'Add Permission');
        if (!roleInfoSubsection) {
          throw new Error('Role Information subsection is missing');
        }

        const departmentField = roleInfoSubsection.fields.find(field => field.name === 'idServiceMaster');
        if (!departmentField) {
