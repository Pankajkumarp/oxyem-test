  ...
  const getAttendanceDetails = async (id) => {
    try {
      ...
      if (response && response.data && response.data.data && response.data.data.details) {
        ...
        setShiftactionDetails(response.data.data.actionDetails)
          ...
          <div className="modal-body  oxyem-attendence-popup">
            ...
            <div className="oxyem-attendence-popup-card oxyem-attendence-popup-card-bottom">
              <div className="card1">
                ...
                <div>{convertUtcToLocalTime(checkInTime, timeZone)}</div>
              </div>
              <div className="card2">
                <span className="sideline"></span>

                <div>Last Check-in</div>
                <div>{convertUtcToLocalTime(checkOutTime, timeZone)}</div>
              </div>
              <div className="card3">
                <span className="sideline"></span>
                <div>Total  Hrs</div>
...
{ShiftactionDetails.map((detail, index) => (
      <div className="main-view-box-leave" key={index}>
        <div className="detail-box-leave">
          

          {detail.actionfor === "attendance" && detail.actionBy  &&(
            <>
              <div className="top-box-other-text top-box-other-text-detail ">
                {detail.status} on {detail.actionOn} by{" "}
                <span>{detail.actionBy}</span>
              </div>
              {detail.status && (
                <div className={`top-box-leave-right leave-${detail.status}`}>
                  {detail.status}
                </div>
              )}
            </>
          ...
          {detail.actionfor === "shift" && detail.actionBy && (
            <>
              <div className="top-box-other-text top-box-other-text-detail">
                Shift Allowance {detail.status} on {detail.actionOn} by {detail.actionBy}
                <span>{detail.shiftactiondBy}</span>
              </div>
              {ShiftData.shiftReason && (
                <span>Reason: {ShiftData.shiftReason}</span>
              )}
              {ShiftData.status && (
                <div className={`top-box-leave-right leave-${detail.status} shift-history-status`}>
                  {ShiftData.status}
                </div>
              )}
            </>
