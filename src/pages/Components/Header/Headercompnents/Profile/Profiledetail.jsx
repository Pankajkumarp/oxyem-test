      ...
      <ul className="nav oxyem-profile-menu" ref={inputRef}>
        <li className="nav-item dropdown noti-dropdown ">
          <span onClick={toggleDropdown} className="dropdown-toggle  nav-link header-nav-list oxyem-profile-link-head" data-bs-toggle="dropdown" data-tooltip-id="my-tooltip-n" data-tooltip-content={"Notification"}>
            <Profile name={profileData.employeeName} imageurl={profileData.profilePicPath} size={"28"} profilelink={"/employeeDashboard"} />
            <span className='main_emp_name'>{truncateName(profileData.employeeName)}</span>
            <span className='main_emp_name_icon'>{isDropdownOpen ? <IoIosArrowUp /> : <IoIosArrowDown />}</span>
          </span>
