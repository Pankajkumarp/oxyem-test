...
import UploadFileDetail from '../Popup/uploadFileDetail.jsx';
...
export default function tablewithApi({ title, ismodule, onEditClick, onSubmitClick, responseData, onDeleteClick, onViewClick, onHistoryClick, handleApprrovereq, handlerecallvalueClick, handleViewAssignReq, pagename, dashboradApi, refreshtable,ifForvalue,updatelist, year ,utctimeconditionpage ,empId ,handleDecommissionreq,assetsparms,refreshAfterEdit ,handleDeallocationreq ,handleSubmitAllocation,checkboxbuttonName ,idBoa, onEmailClick,idJobApplicant, idProject, idSeparation, onConfirmClick ,searchfilter,tableStatus ,tabParamsInObj ,idClaim,status,empType,GetTotalSum,isShowFor,documentFor, onProcessClick, perPage}) {
  ...
  const DelayedCertificateDownload = ({ value, tableMeta }) => {
    const [showIcon, setShowIcon] = useState(false);
  
    useEffect(() => {
      if (tableMeta.rowIndex === 0) {
        const timer = setTimeout(() => setShowIcon(true), 7000); // 7-sec delay
        return () => clearTimeout(timer);
      } else {
        setShowIcon(true); // Show immediately for other rows
      }
    }, [tableMeta.rowIndex]);
  
    return value && value.length > 0 ? (
      <IoDownloadOutline
        style={{ cursor: showIcon ? 'pointer' : 'not-allowed', opacity: showIcon ? 1 : 0.5 }}
        size={20}
        color={showIcon ? '' : '#A0A0A0'}
        onClick={() => showIcon && handleDownloadClickWithPath(value)} // Prevent click when disabled
      />
    ) : null;
  };
  ...
  const fetchData = async (page, rowsPerPage, sortOrder, filterList, searchValue) => {
    ...
    if(response.data.data.totalsum && isShowFor==="view"){  
      const totalSum = response.data.data.totalsum
      const totalCount = response.data.data.totalCount
    GetTotalSum(totalSum, totalCount)
    }
  };
  ...
  useEffect(() => {
    fetchData(page, rowsPerPage, sortOrder, filterList, searchValue);
  }, [page, rowsPerPage, sortOrder, filterList, year, searchValue,assetsparms ,searchfilter ,tableStatus ,tabParamsInObj]);
  ...
  const options = {
    ...
    viewColumns: true, // Enable view/hide columns option
	...
	onDownload: (buildHead, buildBody, columns, data) => {
      const excludedColumnNames = ['Id', 'Action', 'id', 'action', 'idAssignTask', 'Assigned Members','ducumentPath','certificatePath' ,'Uploadfile'];
      ...
      <div className='oxyem-action-buttons'>
        {actions.map((action, index) => {
          ...
          switch (action.type) {
            ...
            case "process":
              button = (
                <span key={index} className='oxyem-without-btn oxyem-mark-process' data-tooltip-id="my-tooltip-datatable" data-tooltip-content={"process"}>
                  <FcProcess />
                </span>
              );
              if (action.isEnable === true) {
                button = React.cloneElement(button, {
                  onClick: () => handleRowprocessClick(tableMeta.rowData, value, updateValue),
                });
              } else {
                button = React.cloneElement(button, {
                  className: `${button.props.className} table_btn_disabled`, // add a disabled class to the button
            ...
            case "aprvrej":
              ...
              case "radio":
                ...
                case "upload":
              button = (
                <span key={index} className='oxyem-without-btn oxyem-mark-upload' data-tooltip-id="my-tooltip-datatable" data-tooltip-content={"Upload Download"}>
                  <FaFileAlt />
                </span>
              );
              if (action.isEnable === true) {
                button = React.cloneElement(button, {
                  onClick: () => handleRowUploadClick(tableMeta.rowData, value, updateValue),
                });
              } else {
                button = React.cloneElement(button, {
                  className: `${button.props.className} table_btn_disabled`, // add a disabled class to the button
...
const renderdocTypeImag = (value, tableMeta, updateValue) => {
  ...
  const rendercustomProfile = (value, tableMeta, updateValue) => {
    ...
    const profilePicture = profileData ? profileData.Profilepicture : profileData.profilePicPath ? profileData.profilePicPath : "";
  ...
  const columns = columnss.map(column => ({
    ...
    options: {
      ...
      customBodyRender: column.name === 'action'
        ...
        : column.name === 'docuSignS3Path'
          ? (value, tableMeta, updateValue) => renderdownloadButon(value, tableMeta, updateValue)
        : column.name === 'docType'
          ? (value, tableMeta, updateValue) => renderdocTypeImag(value, tableMeta, updateValue)
          : column.name === 'warrantyDoc'
            ...
            : column.name === 'leaveReason' || column.name === 'timesheetDescription' || column.name === 'policyDescription' || column.name === 'rewardDescription' || column.name === 'remarks'
...
const getFileName = (path) => {
  ...
  const openUploadDetailpopup = (id) => {
    setDocumentId(id);
    setIsUploadModalOpen(true);
  };
  const closeUploadDetailpopup = async () => {
  ...
  return (
    <>
      ...
      <UploadFileDetail isOpen={isUploadModalOpen} closeModal={closeUploadDetailpopup} documentId={documentId} documentFor={documentFor}/>
