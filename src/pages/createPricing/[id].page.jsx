...
export default function opportunity({ userFormdata }) {  // Default to empty array if not provided
    ...
    const [dataDocuments, setDataDocuments] = useState([]);  // ADD THIS
        ...
        try {
            if (value) {
                ...
                if (response.status === 200 && response.data.data) {
                    ...
                    const dateValue = {
                        endDate: fetchedData.datapricinginfo.endDate,
                        startDate: fetchedData.datapricinginfo.startDate
                    }
                    setOpportunityDate(dateValue)
                    ...
                    opportunitySubsection.fields.forEach(field => {
                        if (fetchedData.datapricinginfo[field.name] !== undefined) {
                            field.value = fetchedData.datapricinginfo[field.name];
                            if (field.name === "projectName") {
                                field.dependentId = fetchedData.datapricinginfo.idClient;
                            }
                            if (field.name === "BTP") {
                            ...
                            const fieldsToDisable = ["opportunityNo", "clientName", "currencyType"];
    ...
    const handleTabClick = (tab) => {
        ...
        if (activeTab !== tab) {
            ...
            } else if (tab === "Documents" && dataDocuments.length === 0) {  // ADD THIS
                setDataDocuments(initializeDataForTab(tab));  // ADD THIS
            }
            // }
        } else {
    ...
    const addRow = () => {
        ...
        } else if (activeTab === "Documents") {  // ADD THIS
            setDataDocuments([...dataDocuments, newRow]);  // ADD THIS
        }
    ...
    useEffect(() => {
        ...
        if (initialSection && initialSection.Subsection.length > 0) {
            ...
            } else if (activeTab === "Documents" && dataDocuments.length === 0) {  // ADD THIS
                setDataDocuments(initializeDataForTab(activeTab));  // ADD THIS
            }
    ...
    const handleDeleteRow = (rowIndex) => {
        ...
        if (activeTab === "Pricing Information") {
            ...
            updatedData = [...dataDocuments];  // ADD THIS
            updatedData.splice(rowIndex, 1);  // ADD THIS
            setDataDocuments(updatedData);  // ADD THIS
        }
    };
    const columns = fields.map(field => ({
        ...
        options: {
            ...
            customBodyRender: (value, tableMeta, updateValue) => {
                ...
                } else if (field.type === 'File') {  // ADD THIS ENTIRE BLOCK
                    return (
                        <span className={error ? "table_field_error" : ""}>
                            <input
                                type="file"
                                ...
                                onChange={(e) => {
                                    ...
                                    if (file) {
                                        ...
                                        handleChangeValue(field.name, file.name, rowIndex);
                                    }
                                }}
                            />
                        </span>
    ...
    const submitformdata = async (value) => {
        ...
        // }
        Object.values(uploadedFiles || {}).forEach((file) => {  // CHANGE FROM: const myFile = value.pricingDoccument
            formData.append('file', file);
        });

        // IF YOU ALSO WANT TO HANDLE FILES FROM FORM:
        const myFile = value.pricingDoccument;  // ADD THIS
        if (Array.isArray(myFile)) {  // ADD THIS
            myFile.forEach((file) => {  // ADD THIS
                formData.append('file', file);  // ADD THIS
            });  // ADD THIS
        } else if (myFile) {  // ADD THIS
            formData.append('file', myFile);  // ADD THIS
        }  // ADD THIS
                    ...
                    <div className="content container-fluid">
                        <div className="row">
                            <div className="col-12 col-lg-12 col-xl-12">
                                <div className="row">
                                    <div className="col">
                                        <Breadcrumbs maintext={"Edit Pricing"} />
                                    </div>
