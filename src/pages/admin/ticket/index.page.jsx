...
export default function Ticket() {
  ...
  useEffect(() => {
    if (activeTab === 1) {
      const chartData = async () => {
        try {
          const apiUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
          const response = await axiosJWT.get(`${apiUrl}/ticket/graphStats`, {
            params: {}
          });
  ...
  const handleTableTabClick = (tabIndex) => {
    setActiveTableTab(tabIndex);
  };
      ...
      <div className="main-wrapper">
        <div className="page-wrapper">
          <div className="content container-fluid">
            <Breadcrumbs maintext={"Dashboard (Ticket)"} addlink={"/admin/ticket/add"} />
              ...
              <div className="col-12 col-lg-12 col-xl-12">
                <div className="card flex-fill comman-shadow oxyem-index mb-4 oxyem-main-graph-sec">
                  ...
                  <div className="tab-content">
                    {activeStatus !== null && <span className='clr-filter-stats' ><MdDeleteForever size={25} 
                    //onClick={() => handleShowDataForStatus('clr')} 
                  /></span>}
                    {activeTab === 0 &&
                      <div>
                          ...
                          <ul className="nav-tabs nav nav-tabs-bottom oxyem-graph-tab">
                            <li className={`nav-item ${activeTableTab === 0 ? 'active' : ''}`}>
                              <a className={`nav-link`} onClick={() => handleTableTabClick(0)}>
                                <div className="skolrup-profile-tab-link">Action Required</div>
                              </a>
                            </li>
                            <li className={`nav-item ${activeTableTab === 1 ? 'active' : ''}`}>
                              <a className={`nav-link`} onClick={() => handleTableTabClick(1)}>
                                <div className="skolrup-profile-tab-link">Resolved / Closed </div>
                              </a>
                            </li>
                          </ul>
