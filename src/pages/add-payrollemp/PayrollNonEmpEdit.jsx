...
export default function PayrollNonEmpEdit() {
            ...
            const getPrefilledData = async () => {
                ...
                try {
                    ...
                    if (response) {
                        ...
                        setcurrency(responseData?.currencySymbols);
                        setPreviewData(prev => ({
                            ...prev,
                            netPay: responseData?.netPay
                        }));

                        if (responseData?.otherAllowance?.length) {
                            setExtraFields(prev => ({
                                ...prev,
                                otherAllowance: responseData.otherAllowance.map(item => ({
                                    description: item.name,
                                    amount: item.attributeValue
                                }))
                            }));
                        }
                        if (responseData?.deductionOtherAllowance?.length) {
                            setExtraFields(prev => ({
                                ...prev,
                                deductionOtherAllowance: responseData.deductionOtherAllowance.map(item => ({
                                    description: item.name,
                                    amount: item.attributeValue
                                }))
                            }));
...
const syncPreviewDynamicFields = (updatedExtraFields) => {
    setPreviewData(prev => ({
        ...prev,
        ...Object.keys(updatedExtraFields).reduce((acc, key) => {
            acc[key] = updatedExtraFields[key].map(row => ({
                name: row.description,
                attributeValue: row.amount
            }));
            return acc;
        }, {})
    }));
    ...
    syncPreviewDynamicFields(updated);
    return updated;
});
...
const deleteSavedRow = (name, index) => {
    setExtraFields(prev => {
        ...
        const payload = buildPayrollPayloadWithExtras(updated);
