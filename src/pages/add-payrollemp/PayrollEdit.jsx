...
import EditPopup from "../add-payroll/EditPopup";
...
export default function PayrollEdit({ allData }) {
    ...
    const [idEmployee, setidEmployee] = useState(allData?.idEmployee || null);
    const [applicableFrom, setApplicableFrom] = useState(allData?.applicableFrom);
    const [previousMonthSalary, setPreviousMonthSalary] = useState(0);
    const [attendanceInfo, setAttendanceInfo] = useState(allData?.attendanceInfo || {});
    ...
    const mergeDynamicExtraFields = (salaryData, content) => {
        if (!salaryData || !content) return {};

        const section = content.section?.[0];
        const earningFields = section?.earningfield?.[0]?.fields || [];
        const deductionFields = section?.deductionsfield?.[0]?.fields || [];

        const mapped = {};

        // 🔹 Earnings (TextwithAdd)
        earningFields.forEach(field => {
            if (field.type === "TextwithAdd" && salaryData[field.name]) {
                mapped[field.name] = salaryData[field.name].map(item => ({
                    description: item.name,
                    amount: item.attributeValue
                }));
            }
        });

        // 🔹 Deductions (TextwithAdd)
    ...
    const fetchForm = async () => {
        try {
            const response = await axiosJWT.get(`${apiUrl}/getDynamicForm`, {
                params: { formType: "payrollNonEmpEditor" },
            });

            if (response.status === 200 && response.data.data) {
                const form = response.data.data;
                const mergedForm = mergeSalaryValues(form, allData);
                setContent(mergedForm);
                const mappedExtras = mergeDynamicExtraFields(allData, mergedForm);
                setExtraFields(mappedExtras);
                setDraftEarnings({});
                setDraftDeductions({});
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        const hasBackendExtras =
            allData?.otherAllowance?.length ||
            allData?.deductionOtherAllowance?.length;
        ...
        try {
            ...
            if (response) {
                ...
                setContent(prev => {
                    ...
                    const section = clone.section?.[0];

                    const updateFields = (fields) => {
                        fields.forEach(field => {
                            if (
                                field.type !== "TextwithAdd" &&
                                responseData[field.name] !== undefined
                            ) {
                                field.value = responseData[field.name];
                            }
                        });
                    };

                    section?.earningfield?.forEach(sub =>
                        updateFields(sub.fields)
                    );
    ...
    const buildPayrollPayloadWithExtras = (extras) => {
        if (!content) return {};
        ...
        Object.keys(extras).forEach(fieldName => {
            payload[fieldName] = (extras[fieldName] || [])
                .filter(row => row.description || row.amount)
                .map(row => ({
                    name: row.description?.label || row.description || null,
                    attributeValue: row.amount || "0"
                }));
        });
    ...
    const saveEarnings = (name) => {
        ...
        // 🔥 Use UPDATED extras for payload
        const payload = buildPayrollPayloadWithExtras(updatedExtras);
        submitformdata(payload);
    };
    ...
    const renderSavedAsField = (rows, label, name) => {
        ...
        return rows.map((row, idx) => (
            <div className="col-md-12" key={`${name}-${idx}`}>
    ...
    const deleteSavedRow = (name, index) => {
        setExtraFields(prev => {
            ...
            // 🔥 build payload using updatedExtras directly
            const payload = buildPayrollPayloadWithExtras(updatedExtras);
            submitformdata(payload);
    ...
    const handleSaveDraft = async () => {
        ...
        payload.netPay = netPayable;
        payload.grossEarnings = totalEarnings;
    ...
    return (
                ...
                <div className="col-xl-8">
                    <div className="card main-card border-0 shadow-sm">
                        ...
                        <div className="row p-4 top-field-i">
                            ...
                            <div className="col-md-6 top-field-add">
                                <MounthPicker onChange={onMonthChange} label="Applicable Month/Year" value={applicableFrom} isDisabled={true} />
                            </div>
                        </div>
                        {idEmployee && (
                            <div className="employee-bar mx-4 mb-4 p-3 rounded-3">
                                ...
                                <div className="d-flex justify-content-between flex-wrap align-items-start">
                                    ...
                                    <div className="d-flex align-items-center gap-3">
                                        <Profile
                                            name={idEmployee?.label}
                                            imageurl={idEmployee?.image}
                                            size={40}
                                            profilelink={idEmployee?.profileLink}
                                        />
                                        <div>
                                    ...
                                    <div className="d-flex gap-4 mt-2 flex-wrap">
                                        ...
                                        <div className="status-item">
                                            <span className={`status-icon ${salaryInfo?.generatedOn ? "completed" : ""}`}>
                                                {salaryInfo?.generatedOn ? <MdOutlineCheckBox /> : <MdOutlineCheckBoxOutlineBlank />}
                                            </span>
                                            <div>
                                                <div className="status-title">Generated</div>
                                                <div className="status-date">
                                                    {salaryInfo?.generatedOn || "Not Generated"}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        {/* Paid */}
                                        <div className="status-item">
                                            <span className={`status-icon ${salaryInfo?.paidOn ? "completed" : ""}`}>
                                                {salaryInfo?.paidOn ? <MdOutlineCheckBox /> : <MdOutlineCheckBoxOutlineBlank />}
                                            </span>
                                            <div>
                                                <div className="status-title">Paid</div>
                                                <div className="status-date">
                                                    {salaryInfo?.paidOn || "Not Paid"}
                                                </div>
                                            </div>
                                        </div>
                                                            ...
                                                            <div className="row g-2 mt-2 mb-2" key={`draft-${idx}`}>
                                                                <div className="col-md-8 top-field-add">
                                                                    <TextField
                                                                        ...
                                                                        onChange={(value) =>
                                                                            updateDraft(field.name, idx, "description", value, "earnings")
                                                                        }
                                                                    />
        ...
        .employee-bar { background: #f3f6fb; border: 1px solid #e4e9f2;}
        .status-item {display: flex;align-items: center;gap: 10px;}
        .status-icon.completed svg{color: #22c55e;font-size: 1.35rem;}
        .status-icon svg{color: #a1a2a2;font-size: 1.35rem;}
        .status-title {font-size: .7rem;font-weight: 600;}
       .status-date {font-size: .6rem;color: #6b7280;}
       .status-message {background: #f3f5fa;color: #374151;padding: 0px 10px;border-radius: .375rem;font-size: .75rem;}
